...
class CollegeRegistrationRenewalController extends Controller
{
    public function college_renewal(){
        $tehsils = Tehsil::get();
        $districts = District::get();
        return view('client.pages.college_registration_renewal', compact('tehsils', 'districts'));
    }

    public function college_renewal_submit(Request $request){
        $rules = [
    ...
    try {
        ...
        $labs = implode(',', $request->input('labs', []));
           ...
           $registration =  CollegeRegistrationRenewal::create(array_merge($request->except('labs'), [
            'labs' => $labs,
            ...
            Mail::to (['test@gmail.com'])->send(new \App\Mail\College_Registration_Renewal($emaildata));
